# katlib — reusable Odin packages for the kat-family apps. Consumed via
# -collection:kat=/path/to/katlib. Each entry is a package directory at the repo
# root; a package earns its slot when a second consumer appears (see README.md).
PACKAGES = text crash gtkenv x11
BRANCH ?= main
REMOTE ?= origin
ROOT_COMMIT_MSG ?= Initial katlib

.PHONY: check test doc push force-push lint check-no-agent-files

check:
	@for p in $(PACKAGES); do echo "== check $$p =="; odin check $$p -collection:kat=. -no-entry-point || exit 1; done

test:
	@for p in $(PACKAGES); do echo "== test $$p =="; odin test $$p -collection:kat=. || exit 1; done

doc:
	@for p in $(PACKAGES); do echo "== doc $$p =="; odin doc $$p -collection:kat=.; done
